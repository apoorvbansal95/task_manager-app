import React from 'react'
import {FaMagnifyingGlass} from"react-icons/fa6"
import {IoMdClose} from "react-icons/io"
export default function SearchBar({value, onChange, handleSearch , onClearSearch}) {
 
 
  return (
    <div className='w-80 flex items-center px-4 bg-slate-100'>
      <input className='w-full text-s bg-transparent py-[11px] outline-none'
       type='text' placeholder='search...'
       value={value}
       onChange={onChange}
       />
    <IoMdClose className='text-slate-300 cursor-pointer hover:text-black mr-3' onClick={onClearSearch}/>
    <FaMagnifyingGlass className=' text-slate-400 cursor-pointer hover:text-black' onClick={handleSearch}/>

    </div>
  )
}
